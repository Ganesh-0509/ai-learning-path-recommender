...
works after every change, the team wrote 60 small programs that each check one specific thing
automatically (e.g. "does marking a course complete actually remove it from future
recommendations?"). All 60 pass, every time, which is how the team can say the app works without
just eyeballing it.
